...
export class SurveyDataComponent implements OnInit {
  ...
  deleteAssignees = (action) => {
    if (action === 'Delete') {
      console.log('checkedUserList at data in if ', this.checkedUserList);
      if (this.checkedUserList) {
        this.displayAssigneesAfterDel(this.checkedUserList);
      }
    }
    console.log('checkedUserList at data', this.checkedUserList);
  }
  ...
  getArrayValue = () => {
    ...
    let userVal;
    this.userArray = [];
    this.allUserArr.filter(users => {
      userVal = this.userArray.includes(users);
      data = this.surveyUserArr.includes(users);
      if (userVal === false) {
        if (data === true) {
          console.log('deleteUserList at get arr val', this.deleteUserList);
      ...
      console.log('userArray at all del', this.userArray);
    });
  }
  editAssignees = (action, obj) => {
    obj.action = action;
    this.getArrayValue();
    console.log('this.userArray at suvry data', this.userArray);
  ...
  addAssignees = (rowObj: any) => {
    const result = [];
    this.userArray = rowObj;
    console.log('userArray after close ass compo', this.userArray);
  ...
  displayAssignees = (userObj: any) => {
    ...
    this.displayAssigneesAfterDel(this.assignees);
  }
  passDataToAss = () => {
    const data = false;
    let index;
  ...
  deleteAllUser = (user) => {
    console.log('checkedUserList at all del', user);
    console.log('isAllSelected == ', this.isAllSelected());
    let index = this.assignees.indexOf(user);
    console.log(index);
  }
